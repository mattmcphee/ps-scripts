...
header_info() {
  clear
  # launc cat -> grab input until whatever is on the right of the << operator is
  # encountered. Send all input that was read-in EXCEPT the thing on the right
  # of the << operator. In this case, the below ascii art will get read in as
  # input to cat which then gets output to the shell.
  cat <<"EOF"
...
# the below four lines use command substitution to make the result of the
# expression inside the $() the contents of the variable
RD=$(echo "\033[01;31m")
YW=$(echo "\033[33m")
GN=$(echo "\033[1;92m")
CL=$(echo "\033[m")
